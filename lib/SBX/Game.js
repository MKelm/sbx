...
SBX.Game.prototype.step = function(scope) {
  ...
  // collision bricks (inside bricks border)
  if (scope.ball.graphic.position.x > scope.bricksBorder.x &&
      scope.ball.graphic.position.x < scope.bricksBorder.x + scope.bricksBorder.w &&
      scope.ball.graphic.position.y > scope.bricksBorder.y &&
      scope.ball.graphic.position.y < scope.bricksBorder.y + scope.bricksBorder.h) {

    var posX = (scope.ball.graphic.position.x - scope.bricksBorder.x) / 30;
    var posY = (scope.ball.graphic.position.y - scope.bricksBorder.y) / 30;

    var posXL = Math.ceil(posX) - 1;
    var posXH = Math.floor(posX) - 1;

    var posYL = Math.ceil(posY) - 1;
    var posYH = Math.floor(posY) - 1;

    if (typeof scope.bricks[posXL] != "undefined" &&
        typeof scope.bricks[posXL][posYL] != "undefined") {
      scope.bricks[posXL][posYL].visible = false;
    }
    if (typeof scope.bricks[posXH] != "undefined" &&
        typeof scope.bricks[posXH][posYH] != "undefined") {
      scope.bricks[posXH][posYH].visible = false;
    }
  }

  // collision paddle
  if (scope.ball.graphic.position.y > scope.spaceBorder.y + scope.spaceBorder.h - 10 &&
