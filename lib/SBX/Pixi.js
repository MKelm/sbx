...
// wrapper class for sbx pixi stuff
...
SBX.Pixi = function() {
  ...
  $(window).resize(function() {
    ...
    sbx.sceneHandler.update();
  });

  var loader = [false, {}];
}

SBX.Pixi.prototype.constructor = SBX.Pixi;

SBX.Pixi.prototype.autoResizeRenderer = function() {
...
SBX.Pixi.prototype.animate = function(scope) {
  if (sbx.userConfig.fps > -1) {
    global.setTimeout(function() { requestAnimFrame(scope.animate.curry(scope)); }, 1000 / sbx.userConfig.fps );
  } else {
...
SBX.Pixi.prototype.setLoader = function(activate) {
  if (activate === true) {
...
SBX.Pixi.prototype.loadAssets = function(callback) {
  /* no graphics yet!!
  this.setLoader(true);
...
SBX.Pixi.prototype.setFpsCounter = function(keyEvent, scope) {
  ...
  if (typeof scope.fps == "undefined") {
    ...
    if (keyEvent === true && scope.fps[0] < 0) {
      ...
      sbx.intervals["fpsCounter"] = setInterval(scope.setFpsCounter.curry(false, scope), 1000);
      keyEvent = false;
    ...
    if (keyEvent === true && scope.fps[0] > -1) {
      clearInterval(sbx.intervals["fpsCounter"]);
      scope.fps[1].visible = false;
      scope.fps[0] = -1;
